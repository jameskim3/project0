...
void CAlign_Interface::OnServerAccept()
{
	...
	else
	{
		...
		SOCKADDR ipsock;
		int ipSockAddrLen; 
		m_pClientSocket->GetPeerName(&ipsock, &ipSockAddrLen); // 상대방의 IP 주소와 포트 번호 얻음
		CString strInfo;
		strInfo.Format(_T("Accept : %s %d"), ipsock.sa_data, ipSockAddrLen);
...
int CAlign_Interface::OnSend(CString strData)
{
	...
	if (nConnectFlag)
	{
		SetMachineStatus(MC_STATUS_NONE);	//메세지를 보냈다는 상태로 설정
